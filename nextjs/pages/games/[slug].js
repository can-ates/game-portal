...
function Game({ game, images, videos, scrollPosition }) {
	...
	const renderIcons = info => {
		switch (info.store.name) {
			...
			case "Nintendo Store":
				return (
					<SiNintendoswitch className={classes.centerColumn__icon} />
				);
			case "Nintendo DS":
			case "Nintendo 3DS":
			case "Nintendo DSi":
					...
					<Grid item container direction='row' spacing={4}>
						...
						>
							<Grid container direction='column'>
								...
								<Grid
									item
									className={classes.centerColumn__date}
								>
									<Skeleton
										animation='pulse'
										variant='rect'
								...
								<Grid
									item
									className={classes.centerColumn__aboutTitle}
								>
									<Skeleton
										animation='pulse'
										variant='rect'
								...
								<Grid
									item
									className={classes.centerColumn__genreTitle}
								>
									<Skeleton
										animation='pulse'
										variant='rect'
								...
								<Grid
									item
									className={
										classes.centerColumn__description
									}
								>
									<Skeleton
										animation='pulse'
						...
						>
							<Grid container direction='column'>
								...
								<Grid
									item
									className={classes.rightColumn__title}
								>
									<Skeleton
										animation='pulse'
										variant='rect'
									...
									<div
										className={
											classes.rightColumn__metascore
										}
									>
										<Skeleton
											animation='pulse'
						...
						<Grid item md={6} className={classes.centerColumn}>
							<Grid container direction='column'>
								...
								<Button
									...
									onClick={() =>
										description.current.style.display ===
										"block"
											? (description.current.style.display =
													"-webkit-box")
											: (description.current.style.display =
					...
					<Slider {...settings}>
						{game.clip && (
							<div className={classes.slider__videoWrapper}>
								...
								/>
								<FaPlay
									className={classes.slider__playButton}
								/>
							</div>
						)}
						...
						{videos.map((video, i) => (
							<div
								key={i}
								className={classes.slider__videoWrapper}
							>
								<video
									src={video.data["480"]}
...
export async function getStaticProps({ params }) {
	...
	const ss = await axios.get(
		`https://api.rawg.io/api/games/${params.slug}/screenshots?key=${process.env.NEXT_PUBLIC_RAWG_KEY}`,
		{
			headers: { "User-Agent": "GamePortal/0.8" },
		}
	);
	...
	const mvs = await axios.get(
		`https://api.rawg.io/api/games/${params.slug}/movies?key=${process.env.NEXT_PUBLIC_RAWG_KEY}`,
		{
			headers: { "User-Agent": "GamePortal/0.8" },
		}
	);
